...
// 地区选择相关
const convertAreaData = (data: any[]) => {
  return data.map(item => {
    const newItem = { ...item, value: item.label };
    if (item.children && item.children.length) {
      newItem.children = convertAreaData(item.children);
    }
    return newItem;
  });
};
const areaOptions = ref(convertAreaData(areaData));
...
const resetSearch = () => {
  ...
  searchForm.address = sea.address = '';
};
...
const searchData = computed(() => {
  ...
  if (sea.address) {
    filteredData = filteredData.filter(item =>
      item.address.includes(sea.address)
    );
  }
