  ...
  <div class="system-settings">
    <el-card class="settings-card">
      <div class="settings-header">
        ...
        <p>{{ t('system.customSettings') }}</p>
      </div>

      <!-- 语言设置区域 -->
      <el-form-item :label="t('system.languageSetting')" class="setting-item">
        <el-select 
      ...
      <div class="account-management">
        <h3>{{ t('system.accountManagement') }}</h3>
        <el-descriptions :column=1 border>
          <el-descriptions-item :label=" t('system.currentUser') ">
            {{ currentUser?.username || t('system.notLoggedIn') }}
          </el-descriptions-item>
          <el-descriptions-item :label=" t('system.lastLoginTime') ">
            {{ currentUser?.lastLogin ? formatDate(currentUser.lastLogin) : t('system.noRecord') }}
          </el-descriptions-item>
        </el-descriptions>
...
const handleLanguageChange = (lang: string) => {
  ...
  ElMessage.success({
    message: lang === 'zh-CN' ? t('message.langSwitchedZh') : t('message.langSwitchedEn'),
    duration: 1500
  });
  // 实际项目中这里会触发全局语言切换
  locale.value = lang;
...
const handleAccountDeletion = () => {
  ...
  ElMessageBox.confirm(
    t('message.confirmDelete', { name: currentUser.value.username }), 
